...
const QuestionsDetails = () => {
  ...
  const handlePostAns = (e, answerLength) => {
    ...
    if (User === null) {
      ...
      if (Answer === "") alert("Enter an answer before submitting");
      else {
        dispatch(postAnswer({id,noOfAnswers: answerLength + 1,answerBody: Answer,userAnswered: User.result.name, answerUserId:User.result._id}));
        setAnswer("");
      }
    }
  };
        ...
        <>
          {questionsList.data.filter((question) => question._id === id).map((question) => (<div key={question._id}>
            ...
            <section className="post-ans-container">
              ...
              <footer>Browse other Question tagged{question.questionTags.map((tag) => (<Link to="/Tags" key={tag} className="ans-tags"> {tag} </Link>))} or 
